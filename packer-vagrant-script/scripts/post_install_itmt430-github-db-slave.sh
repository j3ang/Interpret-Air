...
#
sudo sed  -e "0,/localhost/s/$/    secondary/" /etc/hosts -i
...
# -----------------Done------------------
  ...
  sed  -e "/  bindIp: 0.0.0.0/s/$/, $AIR_DB_SLAVE_IP, 127.0.0.1/" /etc/mongod.conf -i
  ...
  # Set replica name same as master
  # ---------------------------------------------------
  ...
  sudo echo  `systemctl status mongod`
  /usr/bin/mongod --nojournal & while ! nc -vz localhost 27017; do sleep 1; done
  sudo mongo admin --eval 'rs.slaveOk()'
